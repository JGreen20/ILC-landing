...
class User
{
    ...
    {
        if($this->user_id > 0) {
            //return ($this->user_id == (int) $this->CI->session->userdata('user_id')) && ($this->user_role == 3);
    ...
    public function loginGeneral($email, $password, $hash = 'sha256')
    {
        if(empty($email)) {
    ...
    public function loginExtranet($email, $password, $hash = 'sha256')
    {
        if(empty($email)) {
            $this->errors[] = $this->CI->lang->line('user_error_email');
        }
        ...
        $row = $this->_row(array('email' => $email, 'password' => $this->CI->encrypt->password($password, $hash), 'role' => 2));
